use stm_signature::{
    generate_signing_key, public_key_bytes, sign_merkle_root, verify_merkle_root,
...
fn sign_and_verify_merkle_root() {
    ...
    let verifying_key = verifying_key_from_bytes(&public_key).unwrap();

    // Verification must succeed.
    let result = verify_merkle_root(&verifying_key, &merkle_root, &signature);
...
fn modified_merkle_root_fails_verification() {
    ...
    let signature = sign_merkle_root(&signing_key, &original_root);
    ...
    let result = verify_merkle_root(&verifying_key, &modified_root, &signature);
